...
export class PerfilHeaderComponent implements OnInit {
  ...
  constructor(iconRegistry: MatIconRegistry, sanitizer: DomSanitizer,private user:UsuarioService,private route:Router,private modoS:ModoOscuroService) {
  ...
  descargarPDF(event:any){
    console.log(this.per?.usuario.usuario);
    this.evento.emit(event)
   // this.route.navigate(['descargaP',this.per?.usuario.usuario])
  }
  modo(){
